...
def get_footer():
    with ui.footer().classes(
        "flex justify-end items-end text-end text-xs mt-5 bg-transparent text-white font-bold"
    ):
        ui.label("© Minero Pro")
        ui.label("|")
        ui.image(utl.get_github_icon()).classes(
            "w-[16px] h-[16px] hover:opacity-80 cursor-pointer"
        )
        ui.label("|")
