...
def home_page():
    with ui.element('div').classes('grid place-items-center w-full h-[550px]'):
        ...
        with ui.list().classes('grid place-items-center w-full h-full'):
            ui.button('Yacimiento Minero', on_click=lambda: ui.navigate.to(mining_deposit_path)).classes(
                UICommons.button_class)
            ui.button('Plan Minero', on_click=lambda: ui.navigate.to(mining_plan_path)).classes(UICommons.button_class)
            ui.button('Estadísticas', on_click=lambda: ui.navigate.to(statistics_path)).classes(UICommons.button_class)
            ui.button('Cálculos', on_click=lambda: ui.navigate.to(calculations_path)).classes(UICommons.button_class)
